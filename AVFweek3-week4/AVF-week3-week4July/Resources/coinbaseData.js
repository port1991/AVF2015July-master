exports.coinBaseDataFunction = function(){	
	...
	if(typeof data['amount'] !== 'undefined'){*/  
		...
		buttonBtceWebsite.addEventListener('click', function(){  //this will open outlook.com
			var cbWebWin = require('coinbase').btcewebsiteFunct();
			cbWebWin.open();
	...
	mainWin.updateUI = function(data){
		//Api returns single object while local db returns an array so we check for both conditions
		//if data has amount then it means data is available from api or if data has length it means its array returned from local db.
		if(data.amount || data.length)
			currentPrice.text = "1 BTC = "+ data['amount'] + " " + data['currency'];
		else
			currentPrice.text = 'No information found';
	};
	/*
