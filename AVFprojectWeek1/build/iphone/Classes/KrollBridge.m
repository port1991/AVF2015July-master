...
NSString * AVFprojectWeek1$ModuleRequireFormat = @"(function(exports){"
...
@implementation AVFprojectWeek1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_avfprojectweek1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create AVFprojectWeek1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "AVFprojectWeek1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_avfprojectweek1 = [[AVFprojectWeek1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_avfprojectweek1];
	
	NSString *_avfprojectweek1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _avfprojectweek1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_avfprojectweek1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:AVFprojectWeek1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.avfprojectweek1.kroll" 
