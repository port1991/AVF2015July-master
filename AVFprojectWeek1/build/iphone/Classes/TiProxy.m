...
//Rare exceptions to indicate a bug in the _avfprojectweek1 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_avfprojectweek1 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_avfprojectweek1]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.avfprojectweek1.module"
